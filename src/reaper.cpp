...
bool FMMidi::getParameterDisplayValue (VstInt32 index, char* text, float value)
{
	if (!text)
	{
		return false;
...
bool FMMidi::string2parameterReplace (VstInt32 index, char* text)
{
	if (!text)
	{
		return false;
...
bool FMMidi::isEnumParameter (VstInt32 index)
{
	switch (index)
	{
	case kVolumeDisplay:
...
bool FMMidi::automateParameter (VstInt32 index, float value, VstInt32 timestamp)
{
...
bool FMMidi::parameterRange (VstInt32 index, double* range)
{
	if (!range)
	{
		return false;
...
bool FMMidi::renamePlug (char** text, const char* newName)
{
	if (!text || !newName)
	{
		return false;
...
void FMMidi::adjustParameterIndex (VstInt32 index, VstInt32 adjust)
{
