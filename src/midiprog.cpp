...
VstInt32 FMMidi::getMidiProgramName (VstInt32 channel, MidiProgramName* mpn)
{
...
VstInt32 FMMidi::getCurrentMidiProgram (VstInt32 channel, MidiProgramName* mpn)
{
	if (channel < 0 || channel >= 16 || !mpn)
	return -1;
	VstInt32 prg = 0;
...
void FMMidi::fillProgram (VstInt32 channel, VstInt32 prg, MidiProgramName* mpn)
{
...
VstInt32 FMMidi::getMidiProgramCategory (VstInt32 channel, MidiProgramCategory* cat)
{
...
bool FMMidi::hasMidiProgramsChanged (VstInt32 channel)
{
	if (channel < 0 || channel >= 16)
	return false;
	if (synth)
	{
		if (synth->get_program(channel) != channelPrograms[channel])
		{
			return true;
		}
	}
...
bool FMMidi::getMidiKeyName (VstInt32 channel, MidiKeyName* key)
